...
% figure;
% set(gcf, 'Position', [133 106 660 350]);
% xlim([0,30]);
% ylim([-6.3,10.3]);
% box on;
% grid on;
...
for i=1:1:Nt
    ...
    hr=R+r*sin(omega*t);
    xr=v*t+R-r*cos(omega*t);    
    h4=plot(xr,hr,'r.','markersize',10);
    h5=plot(xr,hr,'r*','markersize',7);    
    hc=R+0*sin(omega*t);
    xc=v*t+R-0*cos(omega*t);    
    h6=plot(xc,hc,'k.','markersize',10);
    h7=plot(xc,hc,'k*','markersize',7);        
    xlim([0,xmax]);
%     ylim([-6.2,10.2]);
    axis equal;
    grid on;
